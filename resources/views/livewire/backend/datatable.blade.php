    ...
    <table class="border-collapse table-auto w-full whitespace-no-wrap bg-white table-striped relative">
        ...
        <tbody>
            @foreach ($data as $index => $item)
                <tr x-data="{
                ...
                }" x-cloak class="{{ $tableStriped && $index % 2 != 0 ? 'bg-gray-100' : '' }}">
                    @foreach ($values as $value)
                        <td x-show="!showConfirm" class="border-t border-gray-200">
                            ...
                            @if ($value['type'] === 'data')
                                <span
                                    class="text-gray-700 px-6 py-3 block items-center truncate {{ $value['width'] ?? '' }}">
                                    @if (count($valueItem) == 1)
                                        @if (isset($value['theme']) && $value['theme']['type'] === 'badge')
                                            <span
                                                class="inline-flex font-bold uppercase text-sm tracking-wide px-2 rounded-full {{ $value['theme']['colors'][$item->{$valueItem[0]}] }}">
                                                {{ $item->{$valueItem[0]} }}
                                            </span>
                                        @else
                                            {{ $item->{$valueItem[0]} }}
                                    ...
                                    @if (count($valueItem) == 2)
                                        @if (isset($value['theme']) && $value['theme']['type'] === 'badge')
                                            <span
                                                class="inline-flex font-bold uppercase text-sm tracking-wide px-2 rounded-full {{ $value['theme']['colors'][$item->{$valueItem[0]}->{$valueItem[1]}] }}">
                                ...
                                <div class="text-gray-700 px-6 py-3 flex items-center justify-center">
                                    @if ($actions->contains('edit'))
                                        @if (!empty($editRoute) && !empty($editId))
                                            <a class="transition duration-500 ease-in-out underline underline-indigo-200 text-indigo-500 mr-2"
                                                href="{{ route($editRoute, $item[$editId]) }}">Edit</a>
                                        @else
                                    ...
                                    @if ($actions->contains('delete'))
                                        @if (!empty($deleteRoute) && !empty($deleteId))
                                            <a class="transition duration-300 ease-in-out underline underline-red-200 text-red-500"
                    ...
                    <td x-show="showConfirm" class="border-t border-gray-200"
                        :colspan="showConfirm === true ? '{{ count($values) }}' : 1">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center pt-1">
                                    <span class="shadow-xs mr-2 rounded-lg">
                                        <button type="button" x-on:click="showConfirm = false"
                                            class="px-2 py-1 rounded-lg bg-white text-gray-600">Cancel</button>
                                    </span>

                                    @if ($actions->contains('delete'))
                                        <button x-ref="deleteButton"
